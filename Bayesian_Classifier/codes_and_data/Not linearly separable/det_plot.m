...
miss=0;fpr=0;
for thre=10:-5:-90
        miss=[miss;sum(target<thre)/(size(target,1))];
        fpr=[fpr;sum(sum(nontarget>thre))/(size(nontarget,1)*2)];
end
fpr = fpr - min(fpr(:));
fpr = fpr ./ (max(fpr(:))- min(fpr(:)));
miss = miss - min(miss(:));
miss = miss ./ (max(miss(:))- min(miss(:)));
tp(:,j)=miss(2:end)*100; fp(:,j)=fpr(2:end)*100;
% plot(fpr(2:end),miss(2:end),'-o');
end
...
title('DET curve for non-linear data');
